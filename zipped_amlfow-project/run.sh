...
case "$TARGET" in
  ...
  compare-twoloop-from-files)
    ...
    ;;
  compare-twoloop-fixed-eps)
    # Fresh two-loop check evaluated at one fixed positive epsilon.
    SCRIPT="$PROJECT_DIR/targets/CompareTwoLoopFixedEps.wl"
    ;;
  compare-ksubloop)
    # Cheap full smoke test: all four pieces, same eps order, low precision.
    export AMFLOW_PRECISION_GOAL="${AMFLOW_PRECISION_GOAL:-6}"
